...
task_id: COMP-GLOBAL-HEADER-FOOTER
seq: 39
title: 전역 Header/Footer
category: COMPONENT
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 39)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-064, REQ-FUNC-065)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- [ ] 5개 Screen 공통 내비 4개 링크+계정 아이콘
- [ ] Footer 3컬럼(서비스/회사/이용정책)+면책 문구

## Visual AC

- [ ] 320px~Desktop 반응형, 가로 스크롤 없음
