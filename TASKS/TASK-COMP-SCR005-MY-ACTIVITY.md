...
task_id: COMP-SCR005-MY-ACTIVITY
seq: 37
title: Member: 내 글/참가요청/즐겨찾기/차단
category: COMPONENT
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 37)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-038, REQ-FUNC-040)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.

## Requirement Ref

REQ-FUNC-038, REQ-FUNC-040
...
- `src/components/account/MyActivityPanel.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 내 글 마감/수정/삭제(승인된 요청자 있으면 경고)
- [ ] 참가요청 현황(PENDING/ACCEPTED/REJECTED)
- [ ] 즐겨찾기 목록(localStorage 읽기)
- [ ] 차단 목록+해제

## Visual AC

- [ ] 각 목록 0건 시 완성형 Empty State(안내+CTA)
