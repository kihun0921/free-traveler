...
seq: 7
title: 국내 여행지 Card Grid(Section2)
category: COMPONENT
...
# COMP-SCR001-DOMESTIC-GRID — 국내 여행지 Card Grid(Section2)

## Context

- Category: **COMPONENT** / Implementation Status: **IMPLEMENT** / Priority: **P0**
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 7)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
...
- `src/components/destinations/DomesticGrid.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 국내 탭 선택 시 국내 여행지만 표시(오분류 0건)
- [ ] 카드 클릭 시 `COMP-SCR001-DEST-DRAWER` 오픈

## Visual AC

- [ ] 최소 6개 Card, Desktop 3열/Mobile 1열, 실제 장소 alt 텍스트
