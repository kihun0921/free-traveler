...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 49)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
...
- `src/data/about.ts`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] `50+ Trips`/`30+ Countries`/소개문/철학/30개국 이상/Timeline 6+/추천 여행지 4(공개 여행지 ID 참조)

## Visual AC

- [ ] 이미지 alt+출처만(라이선스 워크플로 없음)
