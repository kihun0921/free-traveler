...
task_id: CI-LINT-TYPECHECK-UNIT
seq: 70
title: CI 파이프라인(Lint/Typecheck/Unit/Build)
category: CI
...
- Category: **CI** / Implementation Status: **IMPLEMENT** / Priority: **P0**
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 70)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: CI

## Project Scope

이 Task가 다루는 Requirement(REQ-NF-031)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- [ ] main 병합 전 ESLint/TypeScript strict/Vitest 단위테스트/`next build` 전부 통과 필수
