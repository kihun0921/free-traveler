...
task_id: COMP-SCR005-AUTH
seq: 35
title: Guest: 로그인/가입/재설정
category: COMPONENT
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 35)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: E2E-MATE-AUTH

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-066)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- `src/components/account/AuthPanel.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 이메일 가입·인증·로그인·로그아웃·비밀번호 재설정
- [ ] 인증되지 않은 이메일은 쓰기 권한 없음

## Visual AC

- [ ] 로그인 후 가능한 기능 체크리스트+보안 안내

## Security/Privacy AC

- [ ] 비밀번호 서버 해시, 콜백은 기술 Route(`/auth/callback`)
