...
task_id: COMP-SCR004-STEPS
seq: 33
title: 신청 방법 3단계(Section5)
category: COMPONENT
...
# COMP-SCR004-STEPS — 신청 방법 3단계(Section5)

## Context

- Category: **COMPONENT** / Implementation Status: **IMPLEMENT** / Priority: **P1**
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 33)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
...
- `src/components/mates/StepsGuide.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 모집글 확인→메시지 작성·제출→승인 후 연결(연락처 비공개 유지) 3단계

## Visual AC

- [ ] 3단계 안내 패턴(다른 Section과 시각 구분)
