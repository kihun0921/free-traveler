...
task_id: RELEASE-VERCEL-SUPABASE-CHECK
seq: 76
title: Vercel/Supabase 배포 확인
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 76)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-NF-012, REQ-NF-016)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- `(문서) `docs/release/deploy-checklist.md`(신규)`

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] Vercel 배포 후 TLS 적용 확인, Supabase 환경변수(서버 전용)가 클라이언트에 노출되지 않음을 빌드 산출물에서 확인
