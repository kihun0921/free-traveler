...
seq: 25
title: 비전달 고지+찾기 Tip 3개(Section5)
category: COMPONENT
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 25)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
...
- `src/components/travel-tools/Tips.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] Tip 3개(날짜 여유/영문 지역명 확인/가격 변동 안내), 3단계 안내 패턴

## Visual AC

- [ ] Card Grid와 다른 시각 패턴(아이콘+텍스트)
