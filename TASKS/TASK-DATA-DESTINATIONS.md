...
priority: P0
depends_on: []
requirements: [REQ-FUNC-007, REQ-FUNC-008, REQ-NF-006, REQ-NF-026]
status: DONE
---
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 47)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: RELEASE-CONTENT-QA

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-007, REQ-FUNC-008, REQ-NF-006, REQ-NF-026)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.

## Requirement Ref

REQ-FUNC-007, REQ-FUNC-008, REQ-NF-006, REQ-NF-026
...
- [ ] 필수 필드(소개300자+/명소5+/추천시기/1·3일 일정/예산/교통/음식3+/에티켓3+/출처/수정일) 스키마로 강제

## Visual AC

- [ ] 이미지는 일반 URL+alt 텍스트만(라이선스 승인 워크플로 없음)

## Security/Privacy AC

- [ ] 출처 URL 없이는 데이터 등록 불가
