...
task_id: COMP-GLOBAL-A11Y
seq: 44
title: 시맨틱 HTML·ARIA 공통 규칙
category: COMPONENT
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 44)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: E2E-PUBLIC-SMOKE(axe-core)

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-079, REQ-NF-023, REQ-NF-024)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.

## Requirement Ref

REQ-FUNC-079, REQ-NF-023, REQ-NF-024
...
- [ ] E2E-PUBLIC-SMOKE(axe-core): Playwright(Chromium)로 핵심 흐름 1개 이상 자동화

## Verify

E2E-PUBLIC-SMOKE(axe-core)
