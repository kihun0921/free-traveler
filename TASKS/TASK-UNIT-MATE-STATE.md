...
task_id: UNIT-MATE-STATE
seq: 65
title: 동행 상태 전이 Unit Test
category: UNIT_TEST
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 65)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Unit(CI)

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-035, REQ-FUNC-036, REQ-FUNC-037)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- `tests/unit/mate-state.test.ts`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] PENDING→ACCEPTED/REJECTED, 중복 요청 차단, 종료일 경과 시 CLOSED 파생 로직 검증
