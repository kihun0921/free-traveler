...
# COMP-SCR001-THEME-CHIPS — 여행 동기·테마 Chip(Section4)

## Context

- Category: **COMPONENT** / Implementation Status: **IMPLEMENT** / Priority: **P1**
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 9)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
...
- `src/components/destinations/ThemeChips.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 테마 6개(자연/도심/미식/휴양/액티비티/가족여행) 선택 시 위 Grid를 같은 화면에서 필터링

## Visual AC

- [ ] Chip 목록 패턴(Card Grid 반복 금지), Desktop/Mobile 모두 wrap
