...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 57)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: UNIT-MATE-STATE, TEST-RLS-BASIC

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-034, REQ-FUNC-035, REQ-FUNC-036, REQ-NF-005)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.

## Requirement Ref

REQ-FUNC-034, REQ-FUNC-035, REQ-FUNC-036, REQ-NF-005
...
- [ ] 500자 제한, 중복 PENDING/ACCEPTED unique 제약, 작성자만 승인/거절
