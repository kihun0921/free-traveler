...
task_id: COMP-GLOBAL-POLICY-PAGES
seq: 46
title: 이용약관·개인정보·안전수칙·면책
category: COMPONENT
implementation_status: IMPLEMENT
screen: 기술 Route(정적 문서)
route: `/policies/*`
page_entry: `src/app/policies/terms/page.tsx` 등
priority: P1
depends_on: []
requirements: [REQ-FUNC-080]
status: TODO
---

# COMP-GLOBAL-POLICY-PAGES — 이용약관·개인정보·안전수칙·면책

## Context

- Category: **COMPONENT** / Implementation Status: **IMPLEMENT** / Priority: **P1**
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 46)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-080)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
| Screen | 기술 Route(정적 문서) |
| Route | `/policies/*` |
| Page Entry | `src/app/policies/terms/page.tsx` 등 |
...
- 해당 Screen: `기술 Route(정적 문서)` — `UI_CONTRACT.md`의 `기술 Route(정적 문서)` 표를 그대로 인용해 AC를 검증한다.
...
- `src/app/policies/terms/page.tsx`(신규)
- `.../privacy/page.tsx`(신규)
- `.../mate-safety/page.tsx`(신규)
- `.../content-disclaimer/page.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 정책 버전 명시, `COMP-SCR003-MATE-WRITE`의 동의 체크박스가 이 버전을 참조
