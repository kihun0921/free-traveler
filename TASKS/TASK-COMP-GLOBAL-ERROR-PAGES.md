...
task_id: COMP-GLOBAL-ERROR-PAGES
seq: 45
title: 404/500/권한없음/외부연결실패
category: COMPONENT
implementation_status: IMPLEMENT
screen: 기술 Route
route: `/404`,`/500`,`/unauthorized`
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 45)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-078)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- 해당 Screen: `기술 Route` — `UI_CONTRACT.md`의 `기술 Route` 표를 그대로 인용해 AC를 검증한다.
...
- `src/app/unauthorized/page.tsx`(신규)
