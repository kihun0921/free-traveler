...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 22)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
...
- `src/components/travel-tools/IntroTabs.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 3단계 미니 안내(탭선택→조건입력→요약확인)
- [ ] 탭 3개 전환 시 아래 Section 콘텐츠만 교체(상태 분리)

## Visual AC

- [ ] 활성 탭 코랄 밑줄, 비활성 muted
