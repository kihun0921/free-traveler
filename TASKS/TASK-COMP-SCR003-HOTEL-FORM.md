...
seq: 24
title: 숙소 조건 입력·요약·외부이동(Section3·4·5)
category: COMPONENT
...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 24)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: UNIT-TRAVEL-DATES, E2E-TRAVEL-TOOLS

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-019, REQ-FUNC-020, REQ-FUNC-021, REQ-FUNC-022, REQ-FUNC-023, REQ-FUNC-024, REQ-FUNC-025, REQ-FUNC-026, REQ-NF-017)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- `src/components/travel-tools/HotelForm.tsx`(신규)
