...
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 14)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-047, REQ-FUNC-048, REQ-FUNC-049, REQ-FUNC-050, REQ-FUNC-051, REQ-FUNC-052, REQ-FUNC-053, REQ-FUNC-054, REQ-NF-028)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- `src/components/destinations/SafetyDrawer.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 8개 필수 카테고리+출처·확인일+외교부 링크(새 탭, noopener,noreferrer)+국가/지역 범위 구분+긴급연락처+면책 고지
- [ ] 7일 초과 시 재확인 필요 경고를 최신 배지보다 우선 노출
- [ ] 중대 경보는 상단 텍스트로 표시

## Visual AC

- [ ] amber(재확인 필요)/critical(중대 경보) 토큰 사용, 코랄 미사용

## Security/Privacy AC

- [ ] 출처 URL 없이는 게시 불가(정적 데이터 검증은 DATA-SAFETY)
