...
task_id: COMP-SCR004-SAFETY-CTA
seq: 34
title: 안전 안내+CTA(Section6)
category: COMPONENT
...
priority: P1
depends_on: []
requirements: [REQ-FUNC-054]
status: TODO
---

# COMP-SCR004-SAFETY-CTA — 안전 안내+CTA(Section6)

## Context

- Category: **COMPONENT** / Implementation Status: **IMPLEMENT** / Priority: **P1**
- 이 Task는 `TASKS/00_TASK_LIST.md`(Seq 34)에서 파생되었으며, 라우트 매핑은 `docs/06_SRS_UIUX_REVISED.md` §2를 따른다.
- Verify 방법: Manual Check

## Project Scope

이 Task가 다루는 Requirement(REQ-FUNC-054)는 `docs/PROJECT_SCOPE.md` 기준 **IMPLEMENT**이다. EXCLUDED Requirement는 이 Task의 범위에 포함되지 않으며, EXCLUDED 목록은 `TASKS/00_TASK_LIST.md` §5(NON_IMPLEMENTATION)를 참조한다.
...
- `src/components/mates/SafetyCtaBanner.tsx`(신규)

> **이 목록 밖의 파일은 수정하지 않는다.** 추가 파일이 필요하면 `TASKS/00_TASK_LIST.md`를 먼저 갱신한 뒤 이 문서를 다시 생성한다.

## Functional AC

- [ ] 안전수칙 요약 3줄+신고·차단 안내+`/travel-tools` CTA
